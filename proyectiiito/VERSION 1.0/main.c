...
printf("\n  -----------------------------------------------------------------");
    ...
    stUsuario buscaPosEnArchivoPersonas(int pos)
{
    stUsuario p;
    int cant;
    ...
    if (fp != NULL)
    {
        ...
        {
            fseek(fp,pos*sizeof(stUsuario),SEEK_SET);
            fread(&p,sizeof(stUsuario),1,fp);
        }
        fclose(fp);
    }
...
int cantidadPersonasEnArchivo()
{
    ...
    if (fp != NULL)
    {
        ...
        cantidadPersonas = ftell(fp)/sizeof(stUsuario);
        fclose(fp);
    }
